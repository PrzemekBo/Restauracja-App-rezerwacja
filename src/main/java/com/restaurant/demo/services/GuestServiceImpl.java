...
@Api(description = "This is GuestServiceImpl class")
@Service
@Transactional
public class GuestServiceImpl implements GuestService {
	...
	@Override
	@ApiOperation(value = "Implementation of the booking method",
	notes = "The process consists in checking whether the guest has entered the appropriate data format, and if the format is incorrect, then throws the appropriate message")
...
// Checking if guest have required values
		...
		String guestNumber = guest.getNumber();
		
		try {
			Integer.parseInt(guest.getNumber());
			 if (guestNumber.length() != 9) throw new Exception();
			 
		}catch(Exception e) {
			return ResponseEntity.status(HttpStatus.NOT_ACCEPTABLE) .body("Please check phone number");
		}
		
		 if(tableEntity != null && (guestSurname.length() >= 4 && guestSurname.length() < 15)){					
		...
		} else return ResponseEntity.status(HttpStatus.NOT_ACCEPTABLE).body("Please check you personal information");
